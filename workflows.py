...
class ReleaseWorkflow:
    ...
    async def run(self, input: ReleaseInfo) -> str:
        ...
        # 1.b - if any deploys are not ready to go, hold
        ...
        deploy_status = await workflow.execute_child_workflow( 
            DeployWorkflow.run,
            DeployInfo(deploy_key = input.release_key+":DEPLOY:sbstore", 
                        container_id = "sbstore", 
                        container_version="1.1", 
                        git_url = "https://github.com/mysampleorg/sbstore.git", 
                        approved = True, 
                        approved_by = "Bob Johnson", 
                        envs = [release_management.production], 
                        release_key = input.release_key,
                        ),
            id=f"deploy-workflow-{input.release_key}:DEPLOY:sbstore:1.1", # todo probably should change this
        )
